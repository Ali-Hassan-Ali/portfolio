<x-dashboard.admin.layout.app>
    ...
    <div class="grid gap-5 lg:gap-7.5">
        ...
        <div class="kt-card kt-card-grid min-w-full kt-card kt-card-accent mb-5">
            ...
            <form method="post" action="{{ route('dashboard.admin.settings.menus.update', $menu) }}">
                @csrf
                @method('PUT')
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-4">
                        <x-input.text name="link" label="global.link" :required="true" :value="$menu->link" />
                        ...
                        <div class="mt-2">
                            ...
                            <select name="type" class="kt-select block w-full">
                                @foreach ($types as $value => $label)
                                    <option value="{{ $value }}" {{ $menu->type === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <x-input.checkbox :label="false" :value="$menu->status" />
